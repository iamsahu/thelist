...
import {  Menu,Dropdown,Image, Button,Responsive,Sidebar,Icon,Accordion } from 'semantic-ui-react'
...
function MenuBar(props){
  ...
  if(content.tags.length>0&&tags===null&&userC.curator_id!==''){
    ...
    if(lastCurator===''||lastCurator!==userC.curator_id){
      ...
      var tagsTemp =content.tags.map(tag=>({ as: "a", content: tag.text, key: tag.key ,href:`/${userC.curator_id}/tags/${tag.value}`}))
      var listsTemp;
      if(content.lists.length>0){
        // console.log(content.lists)
        listsTemp =content.lists.map(list=>({ as: "a", content: list.text, key: list.key ,href:`/${userC.curator_id}/lists/${list.id}`}))
      }
